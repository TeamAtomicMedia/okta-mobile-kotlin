...
class AtomicCoalescingOrchestrator<T : Any>(
    private val factory: suspend (String) -> T,
    ...
    tailrec suspend fun get(factoryArg: String = ""): T {
        if (dataInitialized) {
            return data
        }
        ...
        val result = coroutineScope {
            ...
            synchronized(lock) {
                ...
                if (localDeferred != null && !localDeferred.isCancelled) {
                    ...
                    deferredToAwait = loadDataAsync(this@coroutineScope, factoryArg)
                }
            }
            try {
    ...
    private fun loadDataAsync(scope: CoroutineScope, factoryArg: String): Deferred<T> {
        val local = scope.async(start = CoroutineStart.LAZY) {
            val result = factory(factoryArg)
            synchronized(lock) {
